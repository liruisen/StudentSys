            ...
            if (!String.IsNullOrEmpty(stuId))
            {
                ...
                if (regex.IsMatch(stuId))
                {
                    ...
                    if (stuBll.Delete(sId))
                    {
                        ...
                        context.Response.Redirect("StudentList.aspx?clsId=" + clsId);
                    }
                }
                else
                {

                    context.Response.Write("参数有问题，<a href='ClassList.aspx'>点击这里</a>返回班级列表");

                }
            }
            else
            {
                context.Response.Redirect("ClassList.aspx");
            }
